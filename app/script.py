...
def validate_config():
    ...
    """
    global TP
    if any(not var for var in [USERNAME, PASSWORD, FORMATION, ANNEE, TP]):
...
def submit_attendance(driver, event):
    ...
    """
    # Navigation vers la page Moodle d'émargement
    try:
        ...
        msg = "Échec lors de la recherche du lien de la page Moodle d'émargement."
        logging.error(msg)
        logging.error(e)
    ...
    # Sélection de l'activité 'Présence'
    try:
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        msg = "Échec lors de la recherche de l'activité d'émargement sur la page Moodle d'émargement."
        logging.error(msg)
        logging.error(e)
        logging.info(driver.page_source)
        print(f"[{RED}ERROR{RESET}] {msg}")
...
def perform_emargement(event, scheduled_time_str):
    ...
    submit_attendance(driver, event)
    driver.quit()
